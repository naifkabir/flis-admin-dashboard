...
const studentApproveFormScheam = z
  .object({
    ...
    fees_info: z.array(z.string()).min(1, {
      message: "Required fees group",
    }),
    academic_era: z.string().min(1, {
      message: "Required academic era",
    }),
    boardingStatus: z.string().min(1, {
      message: "Required boarding status",
    }),
  })
  .refine(
    (data) => {
...
const EditStudentForm = ({
  ...
  const boardingStatus = ["DAILY-SCHOLAR", "DAY-BOARDER", "HOSTELLER"];
  // ----------------------------------------------------------------------------------
  ...
  const selectedGroupNames = filteredGroups
  ...
  useEffect(() => {
    if (age !== null && allClasses.length > 0) {
  ...
  const handleSubmit = async (values: FormValues) => {
    const data1 = {
      ...
      student_details: {
        ...
        class: allClasses.find((item: any) => item._id === values.class_info)
          ?.name,
        academic_era: session.find(
          (item: any) => item._id === values.academic_era
        )?.name,
      },
      class_info: values.class_info,
      section_info: values.section_info,
      session_info: values.academic_era,
      fees_info: selectedGroups,
    ...
    try {
      ...
      if (response.statusCode === 200) {
        ...
        window.open(
          `${baseURL}/document/generate-agreement-pdf/${feeStructureId}`,
          "_blank"
        );
                    ...
                    render={({ field }) => (
                      <FormItem
                        ...
                        <Select
                          ...
                          defaultValue={field.value}
                          disabled={
                            !isEditable || !currentDistrictsList.length
                          }>
                          <FormControl>
                            <SelectTrigger className="w-full">
            ...
            <div className="mt-16">
              ...
              <div className="p-9 mt-8 grid md:grid-cols-2 md:gap-x-3 lg:gap-x-8 gap-y-5 md:gap-y-10 bg-[#fff] border-2">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem
                      ...
                      <div className="mt-2 text-[12px] text-gray-600 rounded-sm bg-yellow-400 max-w-fit py-0.5 px-1.5">
                        Student&apos;s Choice of Class: {data?.class}
                      </div>
                      <FormMessage />
                    </FormItem>
                  )}
                      ...
                      <Select onValueChange={field.onChange}>
                        ...
                        <SelectContent>
                          {session?.length > 0 ? (
                            ...
                            <span className="text-black bg-yellow-500 text-[9.5px] px-1 py-0.5 rounded-sm">
                              No Academic Era Found
                            </span>
                          )}
                        </SelectContent>
                      </Select>
                          ...
                          {sectionOptions.length > 0 &&
                            sectionOptions.map((item: any) => (
                              <SelectItem
                                ...
                                value={item._id}
                                disabled={
                                  item.currStudents >= item.maxStudents
                                }>
                  ...
                  render={({ field }) => (
                    <FormItem
                      ...
                      <Select onValueChange={field.onChange}>
                        ...
                        <SelectContent>
                          {boardingStatus.map((item: string, index: number) => (
                ...
                <div className="grid gap-2">
                          ...
                          <FormItem
                            ...
                            <div className="grid items-center gap-x-4">
                              ...
                              <FormControl className="">
                                <input
                                  type="text"
                                  placeholder="Search Fees Group"
                                  value={searchTerm}
                                  onChange={(e) =>
                                    setSearchTerm(e.target.value)
                                  }
                                  className="border rounded-none mb-1 p-2"
                                />
                              </FormControl>

                              <div className="max-h-[163px] border text-[13.5px] overflow-scroll">
                  ...
                  {/* Text area to show selected group names */}
                  <div className="col-span-2">
                    {selectedGroupNames.length > 0 ? (
                      <div className="text-[12px] text-gray-600 rounded-sm bg-yellow-400 max-w-fit py-0.5 px-1.5">
                        Selected Fees Group: {selectedGroupNames}
                      </div>
                    ) : (
                      <div className="text-[12px] text-white rounded-sm bg-red-400 max-w-fit py-0.5 px-1.5">
                        No Fees Group Selected
                      </div>
                    )}
                    {/* <textarea
                      readOnly
                      ...
                      className="w-full h-[38px] border p-2 text-[13px] rounded cursor-not-allowed bg-yellow-400 text-black"
                    /> */}
                  </div>
                </div>
              </div>
