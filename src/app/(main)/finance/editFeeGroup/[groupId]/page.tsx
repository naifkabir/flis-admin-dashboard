...
import PageLoader from "@/components/ui-components/PageLoading";
import { GetGroupById, UpdateFeeGroup } from "@/lib/actions/finance.action";
import { GetAllClasses } from "@/lib/actions/class.action"; // Assuming this API function exists
import { useEffect, useState } from "react";
import { Toaster, toast } from "sonner";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (classes) {
          setClassOptions(
            classes.map((cls: any) => ({ id: cls._id, name: cls.name }))
          );
        }
      ...
      <form
        ...
        <div className="grid grid-cols-3 items-center">
          ...
          <div className="flex flex-col w-full static mb-4 col-span-2">
            <select
              ...
              onChange={handleChange}
              required
              className="border-2 px-[10px] text-black py-[10px] rounded-[5px] w-full focus:outline-none font-semibold font-sans text-[14px] bg-transparent">
        ...
        <div className="flex items-center gap-5 mt-10">
          <button
            ...
            disabled={!isChanged || submitting}>
            {submitting ? "Loading..." : "Update Details"}
          </button>
